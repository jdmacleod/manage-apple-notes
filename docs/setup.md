...
7. **Organization style** — if `settings.local.yaml` was just created, setup asks: how aggressively the AI should reorganize (standard / conservative / static), how many notes trigger a new subfolder, and whether Archive notes should be excluded from classification. Writes `reorganization_mode`, `folder_nesting`, `thresholds.min_notes_for_subfolder`, and `classify.exclude_archive`. Also writes a framework-appropriate `categories:` block — only the roles present in the chosen taxonomy are included, with built-in descriptions and behavioral flags (e.g. `stale_days`, `active_days`). Skipped on re-runs. When the "my current system mostly works" path was chosen, the subfolder threshold defaults to **15 notes** (instead of 8) — a higher bar that reflects the intent to keep the existing structure mostly intact.

8. **Forever Notes Hub structure** — asks whether to enable Hub and Home notes (the `sync-hubs` layer). If yes:
   - Collects the **home note title** (default: `✱ Home`) and **hub note prefix** (default: `✱ `).
   - Asks whether to use **clickable HTML links** (`applenotes://` URLs read via NoteStore.sqlite — requires Full Disk Access for Terminal) or **plain text titles** (always works, default). You can change this later by setting `strict_mode.internal_links` to `"html"` or `"text"` in `settings.local.yaml`. See [docs/forever-notes-framework.md](forever-notes-framework.md) for the tradeoffs.
   - **When a container folder is detected** (e.g. `Library/`): setup tells you where Hub and Home notes will land (the container root by default) and offers a one-field override. Leave blank to accept the default; enter a folder name to place both Hub and Home notes there. Both `home_note_folder` and `hub_note_folder` are set to the same value — override them independently in `settings.local.yaml` if needed.
   
   Writes `forever_notes_mode` and related `strict_mode` keys. Skipped on re-runs.
