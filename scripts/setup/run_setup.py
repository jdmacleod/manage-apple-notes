...
_ROLE_KEYWORDS: dict[str, list[str]] = {
    ...
    "literature": ["literature", "reading", "books", "sources"],
...
def _extract_folders_from_export(export_path: Path) -> tuple[list[str], dict[str, int]]:
    """Return sorted unique folder paths and per-folder note counts from an export."""
    with open(export_path) as f:
        notes = json.load(f)
    counts: dict[str, int] = {}
    for n in notes:
...
def _auto_map_roles(folders: list[str]) -> dict[str, str]:
    """Heuristically assign standard taxonomy role keys to folder paths by leaf name."""
    mapping: dict[str, str] = {}
    for folder in folders:
        leaf = folder.rsplit("/", 1)[-1].lower().strip()
...
def _build_taxonomy_from_export(
    ...
    """Build taxonomy YAML covering all folders in the export.

    Role-matched folders use semantic keys (inbox, projects, …); remaining
    folders get keys derived from their leaf name with collision handling.
    """
    role_mapped_set = set(role_map.values())
    data: dict = {"taxonomy": {}}
    for role, folder in role_map.items():
        data["taxonomy"][role] = {"folder": folder}
    used_keys: set[str] = set(data["taxonomy"])
    for folder in all_folders:
        if folder in role_mapped_set:
            continue
        leaf = folder.rsplit("/", 1)[-1]
        base_key = re.sub(r"[^a-z0-9]+", "_", leaf.lower()).strip("_") or "folder"
        key = base_key
        suffix = 2
        while key in used_keys:
            ...
            suffix += 1
        used_keys.add(key)
        data["taxonomy"][key] = {"folder": folder}
    header = (
        "# taxonomy.local.yaml - generated by `notes setup`\n"
...
def run_setup(dry_run: bool = False, no_corpus: bool = False) -> None:
    ...
    if winner == "EXISTING":
        if export_path is not None:
            all_folders, note_counts = _extract_folders_from_export(export_path)
            role_map = _auto_map_roles(all_folders)
            role_reverse = {v: k for k, v in role_map.items()}
            con.print(f"\n[dim]Found {len(all_folders)} folder(s) in your latest export.[/dim]")
            folder_table = Table(
            ...
            for folder in all_folders:
                role_label = role_reverse.get(folder, "")
                folder_table.add_row(folder, str(note_counts.get(folder, 0)), role_label)
            con.print(folder_table)
            if typer.confirm(
                f"\nGenerate taxonomy from these {len(all_folders)} folder(s)?", default=True
            ):
                taxonomy_yaml = _build_taxonomy_from_export(all_folders, role_map)
            else:
